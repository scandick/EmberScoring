...
def load_company_data():
    ...
    for team in team_states.keys():
        ...
        data[team] = np.clip(scores, 0, 100)
...
def load_employee_data():
    """Generates mock employee data and 30-day historical metrics."""
    names = ["Alice Smith", "Bob Jones", "Charlie Davis", "Diana Evans", "Evan Wright",
             "Fiona Hall", "George King", "Hannah Scott", "Ian Baker", "Julia Adams"]
    teams = ["Engineering", "Engineering", "Engineering", "Marketing", "Marketing",
             "Sales", "Sales", "Customer Support", "Customer Support", "Customer Support"]

    data = []
    for name, team in zip(names, teams):
        # Generate 30 days of data for the 6 metrics
        ember_hist = np.clip(np.random.normal(75, 10, 30), 0, 100)
        prod_hist = np.clip(np.random.normal(80, 8, 30), 0, 100)
        collab_hist = np.clip(np.random.normal(8.5, 1, 30), 0, 10)
        tasks_hist = np.clip(np.random.normal(5, 2, 30), 0, 15).astype(int)
        hours_hist = np.clip(np.random.normal(8, 1.5, 30), 4, 12)
        focus_hist = np.clip(np.random.normal(4, 1.5, 30), 0, 8)

        # Set the current state based on the most recent day (last item in array)
        current_ember = ember_hist[-1]
        status = "At Risk" if current_ember < 60 else ("On Leave" if np.random.rand() > 0.85 else "Active")

        data.append({
            "Name": name,
            "Team": team,
            "Current Ember": int(current_ember),
            "Status": status,
            # Store the historical arrays directly in the dataframe row
            "Ember Score": ember_hist,
            "Productivity": prod_hist,
            "Collaboration": collab_hist,
            "Tasks Completed": tasks_hist,
            "Hours Logged": hours_hist,
            "Focus Time (hrs)": focus_hist
        })
    return pd.DataFrame(data)
...
def draw_dashboard_page():
    ...
    kpi3.metric("Employees At Risk", len(emp_df[emp_df['Status'] == 'At Risk']))
...
def draw_team_overview():
    ...
    for team, state in team_states.items():
        ...
        # Calculate real at_risk from the mock employee dataframe
        at_risk = len(emp_df[(emp_df['Team'] == team) & (emp_df['Status'] == 'At Risk')])
...
def draw_main_team_page():
    ...
    # Filter the global employee dataframe for just this team
    ...
    if team_emps.empty:
        ...
        for idx, row in team_emps.iterrows():
            ...
            # Color code the Ember score
            score_color = "green" if row['Current Ember'] > 80 else (
                "orange" if row['Current Ember'] > 60 else "red")
            c2.markdown(
            ...
            with c3:
                if st.button("View Profile ➡️", key=f"team_emp_btn_{idx}", use_container_width=True):
                    # Hijack the routing state to jump to the employee page!
...
def draw_employees_overview():
    ...
    st.caption("View current status and Ember scores for all employees.")

    # Optional: Add a team filter
    ...
    for idx, row in display_df.iterrows():
        ...
        score_color = "green" if row['Current Ember'] > 80 else ("orange" if row['Current Ember'] > 60 else "red")
        c3.markdown(
            f"{row['Status']} &nbsp;|&nbsp; <span style='color:{score_color}; font-weight:bold;'>{row['Current Ember']}</span>",
            unsafe_allow_html=True)
...
def draw_employee_view():
    ...
    st.write("### 30-Day Performance Trends")

    # Generate the 30 dates to match our arrays
    dates = pd.date_range(start=today - datetime.timedelta(days=29), end=today)

    def draw_mini_chart(metric_col, color):
        # Reconstruct a mini-dataframe for Altair using the dates and the employee's specific array
        chart_df = pd.DataFrame({
            'Date': dates,
            metric_col: emp_details[metric_col]
        })
        ...
        chart = alt.Chart(chart_df).mark_area(
            ...
            x=alt.X('Date:T', axis=alt.Axis(labels=False, title='', ticks=False, grid=False)),
            y=alt.Y(f'{metric_col}:Q', axis=alt.Axis(labels=False, title='', ticks=False, grid=False),
                    scale=alt.Scale(zero=False)),
            tooltip=['Date', alt.Tooltip(f'{metric_col}:Q', format='.1f')]
    ...
    metrics = [
        ("Ember Score", "#FF4B4B"),
        ("Productivity", "#0068C9"),
        ("Collaboration", "#29B09D"),
        ("Tasks Completed", "#FF8700"),
        ("Hours Logged", "#7D44CF"),
        ("Focus Time (hrs)", "#2E7B32")
    ]

    row1_cols = st.columns(3)
    for i in range(3):
        metric_name, color = metrics[i]
        latest_val = emp_details[metric_name][-1]  # Get the last value in the array
        with row1_cols[i]:
            st.write(f"**{metric_name}**")
            st.write(f"### {latest_val:.1f}")
            st.altair_chart(draw_mini_chart(metric_name, color), use_container_width=True)
    ...
    for i in range(3, 6):
        metric_name, color = metrics[i]
        latest_val = emp_details[metric_name][-1]
    ...
    st.info("Recent Feedback / Notes:")
    st.write(f"- Completed quarterly review with {emp_details['Team']} management.")
    st.write(f"- Shows strong upward trend in Focus Time over the last two weeks.")
...
with st.sidebar:
    ...
    # We pass the target page as an argument to our navigate_to function
